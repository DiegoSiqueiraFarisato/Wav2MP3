...
type
  TBladeEncoder = class(TInterfacedObject, IEncoder)
  private
    FHandle: THandle;
    FConfig: TBE_CONFIG;
...
constructor TBladeEncoder.Create;
begin
  inherited Create;
  FHandle := 0;
  FLoaded := False;
...
destructor TBladeEncoder.Destroy;
begin
  UnloadDLL;
  inherited Destroy;
end;

procedure TBladeEncoder.LoadDLL(const DLLPath: string);
begin
  UnloadDLL;
...
procedure TBladeEncoder.UnloadDLL;
begin
  if FHandle > HINSTANCE_ERROR then
  begin
    FreeLibrary(FHandle);
...
procedure TBladeEncoder.Configure(const Config: TEncoderConfig);
begin
...
function TBladeEncoder.InitStream(var Samples: DWORD; var BufferSize: DWORD;
  var Stream: THBE_STREAM): BE_ERR;
begin
  Result := FInitStream(FConfig, Samples, BufferSize, Stream);
end;

function TBladeEncoder.EncodeChunk(Stream: THBE_STREAM; NumSamples: DWORD;
  InBuffer: PSHORT; OutBuffer: PByte; var OutputSize: DWORD): BE_ERR;
begin
  Result := FEncodeChunk(Stream, NumSamples, InBuffer, OutBuffer, OutputSize);
end;

function TBladeEncoder.DeinitStream(Stream: THBE_STREAM; OutBuffer: PByte;
  var OutputSize: DWORD): BE_ERR;
begin
  Result := FDeinitStream(Stream, OutBuffer, OutputSize);
end;

function TBladeEncoder.CloseStream(Stream: THBE_STREAM): BE_ERR;
begin
  Result := FCloseStream(Stream);
end;

function TBladeEncoder.GetVersion: string;
begin
  FGetVersion(FVersion);
  Result := 'Blade MP3 Encoder '
...
function TBladeEncoder.IsLoaded: Boolean;
begin
  Result := FLoaded;
end;
